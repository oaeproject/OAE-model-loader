...
exports.Content = function(batchid, users, worlds) {
    var that = {};
    ...
    for (var i in DISTRIBUTIONS[that.contentType].ROLES){
        ...
        for (var m = 0; m < that.roles[i].totalGroups; m++){
            var randomWorld = nonPrivateGroups[Math.floor(Math.random() * nonPrivateGroups.length)];
            nonPrivateGroups = _.without(nonPrivateGroups, randomWorld);
            that.roles[i].groups.push(randomWorld);
            allmembers.push(randomWorld);
        }
    }
